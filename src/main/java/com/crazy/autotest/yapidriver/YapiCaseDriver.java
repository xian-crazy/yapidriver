package com.crazy.autotest.yapidriver;

import com.crazy.autotest.yapidriver.tools.QuvAssert;
import com.crazy.autotest.yapidriver.yapiwatcher.AllureYapiAdapter;
import com.crazy.autotest.yapidriver.yapiwatcher.Yapi;
import com.crazy.autotest.yapidriver.yapiwatcher.bean.*;
import com.crazy.autotest.yapidriver.allure.QA;
...
public class  YapiCaseDriver {
    ...
    private Map<Integer,InterFaceBean> watcherInterFases(){
        ...
        for(ProjectBean projectBean:allprojectBeans.values()) {
            ...
            //将项目所有用例按所属接口分组
            Map<Integer,List<Integer>> apivcases=new HashMap<>();
...
//        }
        ...
        if ("undone".equals(status) || "testing".equals(status) || "done".equals(status)) {
                String caseids = null == interFaceBean.getCaseIds() ? null : StringUtils.join(interFaceBean.getCaseIds(), ",");
    ...
    private   List<ERB> watcherAllCaseRes(){
        ...
        for(String projectToken:allcaseColBeans.keySet())
            for(CaseColBean caseColBean:allcaseColBeans.get(projectToken)){
                List<ERB> excutResBeansMock = yapi.excutCaseCols(allprojectBeans.get(projectToken), caseColBean, projectToken,allInterFaceBeans, "mock");
                allexcutResBeans.addAll(excutResBeansMock);
                List<ERB> excutResBeansQa = yapi.excutCaseCols(allprojectBeans.get(projectToken), caseColBean, projectToken,allInterFaceBeans, "qa");
...
String status=excutResBean.getInterFaceBean().getStatus();
        if("stoping".equals(status)||"design".equals(status)){
    ...
    private String schema(ERB excutResBean){
        ...
        try {
            ...
            return validateInfo+":非json响应，跳过验证！";
        }
            ...
            try {
                ...
                try {
                    ...
                    schema.validate(res_body);
                }
            } catch (ValidationException | JSONException e) {
                validateInfo="错误：schema校验失败！详细信息见：schema校验详情";
               Allure.descriptionHtml(AllureYapiAdapter.newIt().descriptionERb(excutResBean,e));
            }
        return validateInfo;
    }
